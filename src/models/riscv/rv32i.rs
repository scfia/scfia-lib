...
pub struct ForkSink {
    ...
    pub forks: Vec<Rc<RefCell<dyn ActiveAst>>>,
}
...
pub unsafe fn sum(state: *mut SystemState, _stdlib: &mut ScfiaStdlib, _fork_sink: Option<Rc<RefCell<ForkSink>>>, _memory: &mut Memory32) -> Rc<RefCell<dyn ActiveAst>> {
...
pub unsafe fn step(state: *mut SystemState, _stdlib: &mut ScfiaStdlib, _fork_sink: Option<Rc<RefCell<ForkSink>>>, _memory: &mut Memory32) {
    let instruction_32: Rc<RefCell<dyn ActiveAst>> = _memory.read((*state).pc.clone(), 32, _stdlib);
    let opcode: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 6, 0, _stdlib)));
    if _stdlib.do_eq(opcode.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b11, 7, _stdlib))), _fork_sink.clone()) {
        let funct3: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 14, 12, _stdlib)));
        if _stdlib.do_eq(funct3.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b10, 3, _stdlib))), _fork_sink.clone()) {
            let rd: Rc<RefCell<dyn ActiveAst>> = extract_rd_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
            let rs1: Rc<RefCell<dyn ActiveAst>> = extract_rs1_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
            let imm: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 31, 20, _stdlib)));
            let imm32: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSignExtendExpression::new(imm.clone(), 12, 32, _stdlib)));
            let base_address: Rc<RefCell<dyn ActiveAst>> = register_read_BV32(state, rs1.clone(), _stdlib, _fork_sink.clone(), _memory);
            let address: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVAddExpression::new(base_address.clone(), imm32.clone(), _stdlib)));
            let value: Rc<RefCell<dyn ActiveAst>> = _memory.read(address.clone(), 32, _stdlib);
        ...
        if _stdlib.do_eq(funct3.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b0, 3, _stdlib))), _fork_sink.clone()) {
            let rd: Rc<RefCell<dyn ActiveAst>> = extract_rd_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
            let rs1: Rc<RefCell<dyn ActiveAst>> = extract_rs1_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
            let offset: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 31, 20, _stdlib)));
            let offset_32: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSignExtendExpression::new(offset.clone(), 12, 32, _stdlib)));
            let result: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVAddExpression::new(offset_32.clone(), register_read_BV32(state, rs1.clone(), _stdlib, _fork_sink.clone(), _memory), _stdlib)));
            ...
            let rs1: Rc<RefCell<dyn ActiveAst>> = extract_rs1_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
            let imm: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 31, 20, _stdlib)));
            let imm_32: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSignExtendExpression::new(imm.clone(), 12, 32, _stdlib)));
            let result: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVOrExpression::new(register_read_BV32(state, rs1.clone(), _stdlib, _fork_sink.clone(), _memory), imm_32.clone(), _stdlib)));
            register_write_BV32(state, rd.clone(), result.clone(), _stdlib, _fork_sink.clone(), _memory);
            progress_pc_4(state, _stdlib, _fork_sink.clone(), _memory);
        }
        else {
        ...
        if _stdlib.do_eq(funct3.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b10, 3, _stdlib))), _fork_sink.clone()) {
            let rs1: Rc<RefCell<dyn ActiveAst>> = extract_rs1_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
            let rs2: Rc<RefCell<dyn ActiveAst>> = extract_rs2_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
            let offset_11_5: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 31, 25, _stdlib)));
            let offset_4_0: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 11, 7, _stdlib)));
            let offset: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVConcatExpression::new(offset_11_5.clone(), offset_4_0.clone(), _stdlib)));
            let offset_32: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSignExtendExpression::new(offset.clone(), 12, 32, _stdlib)));
            let base_address: Rc<RefCell<dyn ActiveAst>> = register_read_BV32(state, rs1.clone(), _stdlib, _fork_sink.clone(), _memory);
            let address: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVAddExpression::new(base_address.clone(), offset_32.clone(), _stdlib)));
            let value: Rc<RefCell<dyn ActiveAst>> = register_read_BV32(state, rs2.clone(), _stdlib, _fork_sink.clone(), _memory);
        ...
        let rd: Rc<RefCell<dyn ActiveAst>> = extract_rd_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
        let rs: Rc<RefCell<dyn ActiveAst>> = extract_rs1_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
        let imm: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 31, 12, _stdlib)));
        let value: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVConcatExpression::new(imm.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b0, 12, _stdlib))), _stdlib)));
        register_write_BV32(state, rd.clone(), value.clone(), _stdlib, _fork_sink.clone(), _memory);
        progress_pc_4(state, _stdlib, _fork_sink.clone(), _memory);
    }
    else if _stdlib.do_eq(opcode.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b10111, 7, _stdlib))), _fork_sink.clone()) {
        let dst: Rc<RefCell<dyn ActiveAst>> = extract_rd_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
        let imm: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 31, 12, _stdlib)));
        let imm32: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVConcatExpression::new(imm.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b0, 12, _stdlib))), _stdlib)));
        let sum: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVAddExpression::new(imm32.clone(), (*state).pc.clone(), _stdlib)));
        ...
        let funct3: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 14, 12, _stdlib)));
        let funct7: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 31, 25, _stdlib)));
        let rd: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 11, 7, _stdlib)));
        let rs1: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 19, 15, _stdlib)));
        let rs2: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 24, 20, _stdlib)));
        if _stdlib.do_eq(funct3.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b0, 3, _stdlib))), _fork_sink.clone()) {
        ...
        if _stdlib.do_eq(funct3.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b0, 3, _stdlib))), _fork_sink.clone()) {
            ...
            let lhs: Rc<RefCell<dyn ActiveAst>> = register_read_BV32(state, extract_rs1_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory), _stdlib, _fork_sink.clone(), _memory);
            let rhs: Rc<RefCell<dyn ActiveAst>> = register_read_BV32(state, extract_rs2_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory), _stdlib, _fork_sink.clone(), _memory);
            if _stdlib.do_neq(lhs.clone(), rhs.clone(), _fork_sink.clone()) {
                let imm_4_1: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 11, 8, _stdlib)));
                let imm_10_5: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 30, 25, _stdlib)));
                let imm11: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 7, 7, _stdlib)));
                let imm12: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 31, 31, _stdlib)));
                let imm_4_0: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVConcatExpression::new(imm_4_1.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b0, 1, _stdlib))), _stdlib)));
                let imm_10_0: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVConcatExpression::new(imm_10_5.clone(), imm_4_0.clone(), _stdlib)));
                let imm_11_0: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVConcatExpression::new(imm11.clone(), imm_10_0.clone(), _stdlib)));
                let imm_12_0: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVConcatExpression::new(imm12.clone(), imm_11_0.clone(), _stdlib)));
                let offset: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSignExtendExpression::new(imm_12_0.clone(), 13, 32, _stdlib)));
                let address: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVAddExpression::new((*state).pc.clone(), offset.clone(), _stdlib)));
        ...
        if _stdlib.do_eq(funct3.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b0, 3, _stdlib))), _fork_sink.clone()) {
            let dst: Rc<RefCell<dyn ActiveAst>> = extract_rd_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
            let rs1: Rc<RefCell<dyn ActiveAst>> = extract_rs1_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
            let s1: Rc<RefCell<dyn ActiveAst>> = register_read_BV32(state, rs1.clone(), _stdlib, _fork_sink.clone(), _memory);
            let offset: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 31, 20, _stdlib)));
            let offset_32: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSignExtendExpression::new(offset.clone(), 12, 32, _stdlib)));
            let address: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVAddExpression::new(s1.clone(), offset_32.clone(), _stdlib)));
            let return_address: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVAddExpression::new((*state).pc.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b100, 32, _stdlib))), _stdlib)));
            (*state).pc = address.clone();
        ...
        let imm_20: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 31, 31, _stdlib)));
        let imm_10_1: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 30, 21, _stdlib)));
        let imm_11: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 20, 20, _stdlib)));
        let imm_19_12: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSliceExpression::new(instruction_32.clone(), 19, 12, _stdlib)));
        let offset_10_0: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVConcatExpression::new(imm_10_1.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b0, 1, _stdlib))), _stdlib)));
        let offset_11_0: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVConcatExpression::new(imm_11.clone(), offset_10_0.clone(), _stdlib)));
        let offset_19_0: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVConcatExpression::new(imm_19_12.clone(), offset_11_0.clone(), _stdlib)));
        let offset_20_0: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVConcatExpression::new(imm_20.clone(), offset_19_0.clone(), _stdlib)));
        let offset_32: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVSignExtendExpression::new(offset_20_0.clone(), 21, 32, _stdlib)));
        let address: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVAddExpression::new((*state).pc.clone(), offset_32.clone(), _stdlib)));
        let return_address: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVAddExpression::new((*state).pc.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b100, 32, _stdlib))), _stdlib)));
        let dst: Rc<RefCell<dyn ActiveAst>> = extract_rd_32(instruction_32.clone(), _stdlib, _fork_sink.clone(), _memory);
        register_write_BV32(state, dst.clone(), return_address.clone(), _stdlib, _fork_sink.clone(), _memory);
        (*state).pc = address.clone();
...
pub unsafe fn extract_rd_32(op: Rc<RefCell<dyn ActiveAst>>, _stdlib: &mut ScfiaStdlib, _fork_sink: Option<Rc<RefCell<ForkSink>>>, _memory: &mut Memory32) -> Rc<RefCell<dyn ActiveAst>> {
    return Rc::new(RefCell::new(BVSliceExpression::new(op.clone(), 11, 7, _stdlib)));
}

pub unsafe fn extract_rs1_32(op: Rc<RefCell<dyn ActiveAst>>, _stdlib: &mut ScfiaStdlib, _fork_sink: Option<Rc<RefCell<ForkSink>>>, _memory: &mut Memory32) -> Rc<RefCell<dyn ActiveAst>> {
    return Rc::new(RefCell::new(BVSliceExpression::new(op.clone(), 19, 15, _stdlib)));
}

pub unsafe fn extract_rs2_32(op: Rc<RefCell<dyn ActiveAst>>, _stdlib: &mut ScfiaStdlib, _fork_sink: Option<Rc<RefCell<ForkSink>>>, _memory: &mut Memory32) -> Rc<RefCell<dyn ActiveAst>> {
...
pub unsafe fn progress_pc_4(state: *mut SystemState, _stdlib: &mut ScfiaStdlib, _fork_sink: Option<Rc<RefCell<ForkSink>>>, _memory: &mut Memory32) {
    ...
    let new_pc: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVAddExpression::new(old_pc.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b100, 32, _stdlib))), _stdlib)));
    (*state).pc = new_pc.clone();
}

pub unsafe fn register_write_BV32(state: *mut SystemState, register_id: Rc<RefCell<dyn ActiveAst>>, value: Rc<RefCell<dyn ActiveAst>>, _stdlib: &mut ScfiaStdlib, _fork_sink: Option<Rc<RefCell<ForkSink>>>, _memory: &mut Memory32) {
...
pub unsafe fn register_read_BV32(state: *mut SystemState, register_id: Rc<RefCell<dyn ActiveAst>>, _stdlib: &mut ScfiaStdlib, _fork_sink: Option<Rc<RefCell<ForkSink>>>, _memory: &mut Memory32) -> Rc<RefCell<dyn ActiveAst>> {
    if _stdlib.do_eq(register_id.clone(), Rc::new(RefCell::new(BitVectorConcrete::new(0b0, 5, _stdlib))), _fork_sink.clone()) {
...
pub unsafe fn execute_add32(state: *mut SystemState, destination_id: Rc<RefCell<dyn ActiveAst>>, source1_id: Rc<RefCell<dyn ActiveAst>>, source2_id: Rc<RefCell<dyn ActiveAst>>, _stdlib: &mut ScfiaStdlib, _fork_sink: Option<Rc<RefCell<ForkSink>>>, _memory: &mut Memory32) {
    let s1: Rc<RefCell<dyn ActiveAst>> = register_read_BV32(state, source1_id.clone(), _stdlib, _fork_sink.clone(), _memory);
    let s2: Rc<RefCell<dyn ActiveAst>> = register_read_BV32(state, source2_id.clone(), _stdlib, _fork_sink.clone(), _memory);
    let sum: Rc<RefCell<dyn ActiveAst>> = Rc::new(RefCell::new(BVAddExpression::new(s1.clone(), s2.clone(), _stdlib)));
