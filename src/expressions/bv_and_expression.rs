...
impl BVAndExpression {
    ...
    pub fn new_with_id(id: u64, s1: Rc<RefCell<ActiveValue>>, s2: Rc<RefCell<ActiveValue>>, stdlib: &mut ScfiaStdlib) -> BVAndExpression {
        unsafe {
            let z3_context = stdlib.z3_context;
            let ast = Z3_mk_bvand(stdlib.z3_context, s1.try_borrow().unwrap().get_z3_ast(), s2.try_borrow().unwrap().get_z3_ast());
...
impl RetiredBVAndExpression {
    pub fn clone_to_stdlib(
        ...
        let cloned: Rc<RefCell<RetiredValue>> = unsafe {
            let z3_ast = Z3_mk_bvand(cloned_stdlib.z3_context, cloned_s1_ast, cloned_s2_ast);
            Z3_inc_ref(cloned_stdlib.z3_context, z3_ast);
            RetiredBVAndExpression {
                id: self.id,
                s1_id: self.s1_id,
                s1: cloned_s1,
                s2_id: self.s2_id,
