...
impl BVUnsignedRemainderExpression {
    pub fn new(
    ...
    pub fn new_try_concretize(
        ...
        ActiveValue::BitvectorUnsignedRemainderExpression(Self::new_with_id(stdlib.get_symbol_id(), s1, s2, stdlib)).into()
    }

    pub fn new_with_id(id: u64, s1: Rc<RefCell<ActiveValue>>, s2: Rc<RefCell<ActiveValue>>, stdlib: &mut ScfiaStdlib) -> BVUnsignedRemainderExpression {
        unsafe {
            let z3_context = stdlib.z3_context;
            let ast = Z3_mk_bvurem(stdlib.z3_context, s1.try_borrow().unwrap().get_z3_ast(), s2.try_borrow().unwrap().get_z3_ast());
...
impl Drop for BVUnsignedRemainderExpression {
    fn drop(&mut self) {
        ...
        let retired_expression = Rc::new(RefCell::new(RetiredValue::RetiredBitvectorUnsignedRemainderExpression(
...
impl RetiredBVUnsignedRemainderExpression {
    pub fn clone_to_stdlib(
        &self,
        cloned_active_values: &mut BTreeMap<u64, Rc<RefCell<ActiveValue>>>,
        cloned_retired_values: &mut BTreeMap<u64, Rc<RefCell<RetiredValue>>>,
        ...
        let cloned: Rc<RefCell<RetiredValue>> = unsafe {
            let z3_ast = Z3_mk_bvurem(cloned_stdlib.z3_context, cloned_s1_ast, cloned_s2_ast);
...
impl Drop for RetiredBVUnsignedRemainderExpression {
    fn drop(&mut self) {
