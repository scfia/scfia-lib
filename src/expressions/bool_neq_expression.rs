...
impl BoolNEqExpression {
    ...
    pub fn new_with_id(id: u64, s1: Rc<RefCell<ActiveValue>>, s2: Rc<RefCell<ActiveValue>>, stdlib: &mut ScfiaStdlib) -> BoolNEqExpression {
        unsafe {
            let z3_context = stdlib.z3_context;
            let ast = Z3_mk_not(
                z3_context,
                Z3_mk_eq(z3_context, s1.try_borrow().unwrap().get_z3_ast(), s2.try_borrow().unwrap().get_z3_ast()),
            );
...
impl RetiredBoolNEqExpression {
    pub fn clone_to_stdlib(
        ...
        let neg_expression: Rc<RefCell<RetiredValue>> = unsafe {
            let z3_ast = Z3_mk_not(cloned_stdlib.z3_context, Z3_mk_eq(cloned_stdlib.z3_context, cloned_s1_ast, cloned_s2_ast));
