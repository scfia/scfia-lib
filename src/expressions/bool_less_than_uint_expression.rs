    ...
    pub fn new_try_concretize(
        ...
        match s1.try_borrow().unwrap().deref() {
            ActiveValue::BitvectorConcrete(e1) => match s2.try_borrow().unwrap().deref() {
                ActiveValue::BitvectorConcrete(e2) => return BoolConcrete::new(e1.value < e2.value, stdlib, fork_sink).into(),
                _ => {}
            },
            _ => {}
        }
        ActiveValue::BoolLessThanUIntExpression(Self::new_with_id(stdlib.get_symbol_id(), s1, s2, stdlib)).into()
    }

    pub fn new_with_id(id: u64, s1: Rc<RefCell<ActiveValue>>, s2: Rc<RefCell<ActiveValue>>, stdlib: &mut ScfiaStdlib) -> BoolLessThanUIntExpression {
        unsafe {
            let z3_context = stdlib.z3_context;
            let ast = Z3_mk_bvult(stdlib.z3_context, s1.try_borrow().unwrap().get_z3_ast(), s2.try_borrow().unwrap().get_z3_ast());
...
impl Drop for BoolLessThanUIntExpression {
    fn drop(&mut self) {
        ...
        let retired_expression = Rc::new(RefCell::new(RetiredValue::RetiredBoolLessThanUIntExpression(
...
impl RetiredBoolLessThanUIntExpression {
    pub fn clone_to_stdlib(
        ...
        let cloned: Rc<RefCell<RetiredValue>> = unsafe {
            let z3_ast = Z3_mk_bvult(cloned_stdlib.z3_context, cloned_s1_ast, cloned_s2_ast);
