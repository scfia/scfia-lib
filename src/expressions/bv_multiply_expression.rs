...
impl BVMultiplyExpression {
    ...
    pub fn new_try_concretize(
        ...
        ActiveValue::BitvectorMultiplyExpression(Self::new_with_id(stdlib.get_symbol_id(), s1, s2, stdlib)).into()
    }

    pub fn new_with_id(id: u64, s1: Rc<RefCell<ActiveValue>>, s2: Rc<RefCell<ActiveValue>>, stdlib: &mut ScfiaStdlib) -> BVMultiplyExpression {
        unsafe {
            let z3_context = stdlib.z3_context;
            let ast = Z3_mk_bvmul(stdlib.z3_context, s1.try_borrow().unwrap().get_z3_ast(), s2.try_borrow().unwrap().get_z3_ast());
...
impl Drop for BVMultiplyExpression {
    fn drop(&mut self) {
        ...
        let retired_expression = Rc::new(RefCell::new(RetiredValue::RetiredBitvectorMultiplyExpression(RetiredBVMultiplyExpression {
            id: self.id,
            s1_id,
...
impl RetiredBVMultiplyExpression {
    pub fn clone_to_stdlib(
        &self,
        cloned_active_values: &mut BTreeMap<u64, Rc<RefCell<ActiveValue>>>,
        cloned_retired_values: &mut BTreeMap<u64, Rc<RefCell<RetiredValue>>>,
        ...
        let cloned: Rc<RefCell<RetiredValue>> = unsafe {
            let z3_ast = Z3_mk_bvmul(cloned_stdlib.z3_context, cloned_s1_ast, cloned_s2_ast);
            Z3_inc_ref(cloned_stdlib.z3_context, z3_ast);
            RetiredBVMultiplyExpression {
                id: self.id,
                s1_id: self.s1_id,
                s1: cloned_s1,
                s2_id: self.s2_id,
...
impl Drop for RetiredBVMultiplyExpression {
    fn drop(&mut self) {
