...
impl BVXorExpression {
    ...
    pub fn new_try_concretize(
            ...
            ActiveValue::BitvectorConcrete(e1) => {
                match s2.try_borrow().unwrap().deref() {
                    ActiveValue::BitvectorConcrete(e2) => {
                        ...
                        return BitVectorConcrete::new(xor, e2.width, stdlib, fork_sink);
                    }
                    _ => {}
                }
            }
        ...
        ActiveValue::BitvectorXorExpression(Self::new_with_id(stdlib.get_symbol_id(), s1, s2, stdlib)).into()
    }

    pub fn new_with_id(id: u64, s1: Rc<RefCell<ActiveValue>>, s2: Rc<RefCell<ActiveValue>>, stdlib: &mut ScfiaStdlib) -> BVXorExpression {
        unsafe {
            let z3_context = stdlib.z3_context;
            let ast = Z3_mk_bvxor(stdlib.z3_context, s1.try_borrow().unwrap().get_z3_ast(), s2.try_borrow().unwrap().get_z3_ast());
...
impl RetiredBVXorExpression {
    pub fn clone_to_stdlib(
        ...
        let cloned: Rc<RefCell<RetiredValue>> = unsafe {
            let z3_ast = Z3_mk_bvxor(cloned_stdlib.z3_context, cloned_s1_ast, cloned_s2_ast);
            Z3_inc_ref(cloned_stdlib.z3_context, z3_ast);
            RetiredBVXorExpression {
                id: self.id,
                s1_id: self.s1_id,
                s1: cloned_s1,
                s2_id: self.s2_id,
