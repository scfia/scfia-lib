        ...
        match s1.try_borrow().unwrap().deref() {
            ActiveValue::BitvectorConcrete(e1) => match s2.try_borrow().unwrap().deref() {
                ActiveValue::BitvectorConcrete(e2) => {
                    ...
                    return BitVectorConcrete::new(value, e2.width + e1.width, stdlib, fork_sink);
                }
                _ => {}
            },
            _ => {}
        }
        ActiveValue::BitvectorConcatExpression(Self::new_with_id(stdlib.get_symbol_id(), s1, s2, stdlib)).into()
    }

    pub fn new_with_id(id: u64, s1: Rc<RefCell<ActiveValue>>, s2: Rc<RefCell<ActiveValue>>, stdlib: &mut ScfiaStdlib) -> BVConcatExpression {
        unsafe {
            let z3_context = stdlib.z3_context;
            let ast = Z3_mk_concat(stdlib.z3_context, s1.try_borrow().unwrap().get_z3_ast(), s2.try_borrow().unwrap().get_z3_ast());
            Z3_inc_ref(z3_context, ast);
            let depth = 1 + std::cmp::max(s1.try_borrow().unwrap().get_depth(), s2.try_borrow().unwrap().get_depth());
            let width = s1.try_borrow().unwrap().get_width() + s2.try_borrow().unwrap().get_width();
            BVConcatExpression {
                id: id,
                s1: s1,
                s2: s2,
...
impl RetiredBVConcatExpression {
    pub fn clone_to_stdlib(
        &self,
        cloned_active_values: &mut BTreeMap<u64, Rc<RefCell<ActiveValue>>>,
        cloned_retired_values: &mut BTreeMap<u64, Rc<RefCell<RetiredValue>>>,
        ...
        let cloned: Rc<RefCell<RetiredValue>> = unsafe {
            let z3_ast = Z3_mk_concat(cloned_stdlib.z3_context, cloned_s1_ast, cloned_s2_ast);
            Z3_inc_ref(cloned_stdlib.z3_context, z3_ast);
            RetiredBVConcatExpression {
                id: self.id,
                s1_id: self.s1_id,
                s1: cloned_s1,
                s2_id: self.s2_id,
